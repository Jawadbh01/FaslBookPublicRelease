...
export default function LedgerReportPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const list: Entry[] = snap.docs
        .map((d) => {
          const raw = d.data();
          const type = (raw.type === "credit" || raw.type === "debit")
            ? (raw.type as "credit" | "debit")
            : (raw.direction === "credit" ? "credit" : "debit") as "credit" | "debit";
          const category = (raw.category as string) || "other";
          const categoryLabel =
            (raw.categoryLabel as string) ||
            (type === "credit"
          ...
          return {
            ...
            parcelName: (raw.parcelName as string) || "",
            dealerName: (raw.dealerName as string) || "",
    ...
    <div className="min-h-screen pb-24" style={{ backgroundColor: "#F5F5F5" }}>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {entry.notes && (
                      <p className="text-gray-400 text-xs truncate">{entry.notes}</p>
                    )}
                  </div>
                  <p
                    className="font-bold text-sm shrink-0"
                    style={{ color: isCredit ? "#1B5E20" : "#B71C1C" }}
                  >
      ...
      <div className="print-only" style={{ padding: "0 24px" }}>
        <div style={{ borderBottom: "3px solid #1B5E20", paddingBottom: 10, marginBottom: 16 }}>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: "#1B5E20", margin: 0 }}>
            {organization?.name || "Farm"} — Khata Report
          </h1>
        ...
        <table style={{ width: "100%", marginBottom: 20 }}>
          <tbody>
            <tr>
              <td style={{ fontWeight: 600, width: 160, padding: "3px 0", fontSize: 12 }}>Total Credit</td>
              <td style={{ color: "#1B5E20", fontWeight: 700, fontSize: 13 }}>{fmt(totalCredit)}</td>
            </tr>
            <tr>
              <td style={{ fontWeight: 600, width: 160, padding: "3px 0", fontSize: 12 }}>Total Debit</td>
              <td style={{ color: "#B71C1C", fontWeight: 700, fontSize: 13 }}>{fmt(totalDebit)}</td>
            </tr>
            <tr>
              <td style={{ fontWeight: 600, width: 160, padding: "3px 0", fontSize: 12 }}>Net Balance</td>
              <td style={{ color: netBalance >= 0 ? "#1B5E20" : "#B71C1C", fontWeight: 700, fontSize: 14 }}>
                {fmt(netBalance)}
              </td>
            </tr>
          </tbody>
        </table>
        <h2 style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, color: "#333" }}>
          ...
          <tbody>
            {entries.map((e, i) => (
              <tr key={e.id}>
                ...
                <td style={{ color: e.type === "credit" ? "#1B5E20" : "#B71C1C", fontWeight: 600 }}>
                  {e.type === "credit" ? "Credit" : "Debit"}
                </td>
